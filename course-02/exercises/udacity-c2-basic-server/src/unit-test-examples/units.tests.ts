...
describe('concat', () => {
  it('should concat two strings', () => {
    const result = concat("abs", "bs");
    expect(result).to.equal("absbs");
  });

  it('should throw error when a string is empty', () => {
    expect(() => concat("abs", " ")).to.throw('Strings should not be empty');
  });
});